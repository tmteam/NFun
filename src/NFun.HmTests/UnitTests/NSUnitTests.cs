...
namespace NFun.HmTests.UnitTests
{
    public class NSUnitTests
    {
        ...
        {
            _hmNode.SetStrict(0, TiType.ArrayOf(TiType.Generic(0)));
            var result = _hmNode.Solve();
            Assert.AreEqual(1,result.GenericsCount);
        ...
        public void SingleArray()
        {
            ...
            Assert.AreEqual(TiType.ArrayOf(TiType.Int32), _hmNode.Solve().GetNodeType(0));
        }
        ...
        {
            _hmNode.SetStrict(0, TiType.ArrayOf(TiType.Generic(0)));
            Assert.IsTrue(_hmNode.SetStrict(0, TiType.ArrayOf(TiType.Int32)));
        ...
        {
            _hmNode.SetStrict(0, TiType.Generic(0));
            Assert.IsTrue(_hmNode.SetStrict(0, TiType.Int32));
            var solving = _hmNode.Solve();
            Assert.AreEqual(0, solving.GenericsCount);
        ...
        {
            _hmNode.SetStrict(0,  TiType.GenericFun(3));
        ...
        {
            _hmNode.SetCall(new CallDefenition(TiType.Generic(0), new[] {0, 1}));
        ...
        {
            _hmNode.SetStrict(0, TiType.Bool);
            _hmNode.Unite(1, 0);  
            _hmNode.Unite(2, 1);  
            Assert.AreEqual(TiType.Bool, _hmNode.Solve().GetNodeType(2));
        }
        [Test]
        public void TwoNodesEqualsToEachOther_TypeSetToTopNode_TypeIsSame()
        {
            _hmNode.Unite(1, 0);  
            _hmNode.Unite(2, 1);  
            _hmNode.SetStrict(2, TiType.Bool);
            Assert.AreEqual(TiType.Bool, _hmNode.Solve().GetNodeType(0));
            Assert.AreEqual(TiType.Bool, _hmNode.Solve().GetNodeType(1));
            Assert.AreEqual(TiType.Bool, _hmNode.Solve().GetNodeType(2));
        }
        [Test]
        public void TwoArrayNodesEqualsToEachOther_TypeIsSame()
        {
            ...
            _hmNode.Unite(1, 0);  
            _hmNode.Unite(2, 1);  
            Assert.AreEqual(TiType.ArrayOf(TiType.Int32), _hmNode.Solve().GetNodeType(2));
        }
        [Test]
