...
public class TestHardcoreApiBuildInterpolation {
    ...
    public void SingleVariableInterpolation(object input, string expr, string expected) {
        var calculator = Funny.Hardcore.BuildInterpolation(expr);
    ...
    public void ConstantInterpolation(string expr, string expected) => Assert.AreEqual(expected, Funny.Hardcore.BuildInterpolation(expr).Calculate());
}
