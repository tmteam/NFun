...
namespace NFun.Tic.Tests
{
    class TrickyPrimitives
    {
        ...
        [TestCase(PrimitiveTypeName.I32)]
        [TestCase(PrimitiveTypeName.I64)]
        public void ArithFunctionWithPreferedConstant(PrimitiveTypeName preferedType)
        {
            TraceLog.IsEnabled = true;
            //node |    0 2 1        
            //expr |a = 3 * x;    
            ...
            Assert.AreEqual(preferedType, ((ConstrainsState)generic.State).Prefered.Name,
                "Generic state has to prefer preferered type");
        }

        [TestCase(PrimitiveTypeName.Real)]
        [TestCase(PrimitiveTypeName.I32)]
        [TestCase(PrimitiveTypeName.I64)]
        public void ArithFunctionWithPreferedConstantInSecondPosition(PrimitiveTypeName preferedType)
        {
            TraceLog.IsEnabled = true;
            //node |    0 2 1        
            //expr |a = x * 3;    
            ...
            var graph = new GraphBuilder();
            graph.SetVar("x", 0);
            graph.SetIntConst(1, StatePrimitive.U8, StatePrimitive.Real, new StatePrimitive(preferedType));
            graph.SetArith(0, 1, 2);
            graph.SetDef("a", 2);
            graph.Solve();

            var result = graph.Solve();
            var generic = result.AssertAndGetSingleArithGeneric().State;
            Assert.AreEqual(preferedType, ((ConstrainsState)generic).Prefered.Name,
                "Generic state has to prefer preferered type");
        }
    }
}