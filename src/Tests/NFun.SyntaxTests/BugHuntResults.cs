...
public class BugHuntResults {
    ...
    // ── Master bug-hunt 2026-07-02 (float focus) ─────────────────────────────
    // Bug #6 — inline struct + optional narrowing-numeric field + none.
    // `arr:{v:float32?}[] = [{v=1.5}, {v=none}]` threw FU719 pre-merge.
    // Fixed on lang-mutable-collections by Rule B (canonical Optional form) +
    // the opt(P) ∨ CS(Pref) join rule — un-ignored at the master merge.
    [Test]
    public void MasterBug6_InlineStructOptionalFloat32Field_WithNone_ShouldBuild() =>
        Assert.DoesNotThrow(() =>
            "arr:{v:float32?}[] = [{v=1.5}, {v=none}]".BuildWithFloatsAndOptional());

    // ── Bug-hunt 2026-07-03 on master (pre-push run on b89f3183) ─────────────
