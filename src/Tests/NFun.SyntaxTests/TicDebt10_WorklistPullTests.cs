...
public class TicDebt10_WorklistPullTests {
    ...
    // ────────────────────────────────────────────────────────────────
    // Bug47/49 FAMILY surface — all .map(rule it.G()) where G returns
    // generic-T-shaped output. Pinned to confirm the surface is broad
    // (12+ collection operators), not just first/last/toArray/toSet.
    // ────────────────────────────────────────────────────────────────

    [Test]
    public void Bug47Family_MapItLast_WidensToAny() {
        var rt = Funny.Hardcore.BuildLang("out = [[1,2],[3,4]].map(rule it.last())");
        rt.Run();
        Assert.AreEqual("fixedArray<Int32>", rt["out"].Type.ToString());
    }

    [Test]
    public void Bug47Family_MapItToList_WidensElementToAny() {
        var rt = Funny.Hardcore.BuildLang("x = [[1,2,3]].map(rule it.toList())");
        rt.Run();
        Assert.AreEqual("fixedArray<list<Int32>>", rt["x"].Type.ToString());
    }

    [Test]
    public void Bug47Family_MapItToFixedArray_WidensElementToAny() {
        var rt = Funny.Hardcore.BuildLang("x = [[1,2,3]].map(rule it.toFixedArray())");
        rt.Run();
        Assert.AreEqual("fixedArray<fixedArray<Int32>>", rt["x"].Type.ToString());
    }

    [Test]
    public void Bug47Family_MapItReverse_WidensElementToAny() {
        var rt = Funny.Hardcore.BuildLang("x = [[1,2,3]].map(rule it.reverse())");
        rt.Run();
        Assert.AreEqual("fixedArray<fixedArray<Int32>>", rt["x"].Type.ToString());
    }

    [Test]
    public void Bug47Family_MapItTake_WidensElementToAny() {
        var rt = Funny.Hardcore.BuildLang("x = [[1,2,3]].map(rule it.take(2))");
        rt.Run();
        Assert.AreEqual("fixedArray<fixedArray<Int32>>", rt["x"].Type.ToString());
    }

    [Test]
    public void Bug47Family_MapItSkip_WidensElementToAny() {
        var rt = Funny.Hardcore.BuildLang("x = [[1,2,3]].map(rule it.skip(1))");
        rt.Run();
        Assert.AreEqual("fixedArray<fixedArray<Int32>>", rt["x"].Type.ToString());
    }

    [Test]
    public void Bug47Family_FilterThenMapItFirst_WidensToAny() {
        var rt = Funny.Hardcore.BuildLang(
            "out = [[1,2,3]].filter(rule it.count()>0).map(rule it.first())");
        rt.Run();
        Assert.AreEqual("fixedArray<Int32>", rt["out"].Type.ToString());
    }
