...
public class BugHuntStatementsResults_Round4 {
    ...
    // ───────────────────────────────────────────────────────────────
    // StmtBug59 counterpart — a DECLARED `any` field accepts everything;
    //   the widen-guard must not fire on it (was a false positive: the
    //   inlined field guard lacked the declared-any exemption the
    //   variable guard has — review N3, fixed via ReassignmentGuard).
    // ───────────────────────────────────────────────────────────────
    [Test]
    public void FieldAssign_DeclaredAnyField_AcceptsAnyValue() {
        var rt = BuildLang(
            "type t = {v: any}\n" +
            "x = t{v=1}\n" +
            "x.v = 'text'\n" +
