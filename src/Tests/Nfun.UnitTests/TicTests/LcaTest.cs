...
public class LcaTest {
    ...
    public void ArrayOfPrimitiveTypeAndArrayOfConstrainType_ReturnsArrayOfLca() {
        foreach (var primitiveTriple in _primitiveTypesLca)
            AssertLca(StateArray.Of(primitiveTriple.Item1), StateArray.Of(new ConstrainsState()),
                StateArray.Of(primitiveTriple.Item1));
    }
    ...
    public void ArrayOfPrimitiveTypeAndArrayOfConstrainTypeWithDesc_ReturnsArrayOfLca() {
        foreach (var primitiveTriple in _primitiveTypesLca)
            AssertLca(
                StateArray.Of(primitiveTriple.Item1),
                StateArray.Of(TicNode.CreateNamedNode("r", new ConstrainsState(desc: primitiveTriple.Item2))),
                StateArray.Of(primitiveTriple.Item3));
    }
    ...
    public void ComplexNestedArray() {
        foreach (var primitiveTriple in _primitiveTypesLca)
        {
            var array1 = StateArray.Of(
                new StateRefTo(TicNode.CreateNamedNode("foo",
                    StateArray.Of(TicNode.CreateNamedNode("r",
                        new ConstrainsState(
                            desc: primitiveTriple.Item1))))));
            var array2 = StateArray.Of(
                StateArray.Of(primitiveTriple.Item2));

            AssertLca(array1, array2, StateArray.Of(StateArray.Of(primitiveTriple.Item3)));
        }
    }
