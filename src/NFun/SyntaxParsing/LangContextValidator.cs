...
internal static class LangContextValidator {
    ...
    private static bool DefinitelyReturns(ISyntaxNode node) {
        switch (node) {
            ...
            case IfThenElseSyntaxNode ite:
                // The else slot holds an auto-inserted DefaultValueSyntaxNode
                // when no `else` was written — that path falls through.
                // A user-written `else default` is a real value and counts.
                if (ite.ElseExpr is DefaultValueSyntaxNode { IsAutoInsertedElse: true }) return false;
    ...
    private static void ValidateExpressionPosition(ISyntaxNode expr) {
        switch (expr) {
            // Distinguish auto-inserted sentinel (no user else) from a
            // user-written `else default` expression — check the
            // IsAutoInsertedElse flag, not just the node type. (MR11Bug2.)
            case IfThenElseSyntaxNode ite
                when ite.ElseExpr is DefaultValueSyntaxNode { IsAutoInsertedElse: true }:
