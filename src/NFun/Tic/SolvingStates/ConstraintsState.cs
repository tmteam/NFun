...
public class ConstraintsState : ITicNodeState {
        ...
        switch (type)
        {
            ...
            case ICompositeState:
            {
                ...
                if (!type.IsSolved || !Descendant.IsSolved)
                    return false;
                // The target is acceptable iff the interval's lower bound converts into
                // it. A C#-class-only comparison here was both too loose (list<I32> →
                // set<text>: same class, disjoint kind/element) and too strict (the
                // array-branch SC ≤ StateArray bridge that Pull/Push/Fit accept).
                // Unlike Fit, RESOLUTION forbids the implicit lift T ≤ opt(T): a
                // non-optional interval does not silently resolve into opt(T) — the
                // Optional axis must come from the IsOptional flag (or an already-
                // optional lower bound).
                if (type is StateOptional optTarget) {
                    if (Descendant is StateOptional descOpt)
                        return Algebra.StateExtensions.FitsInto(descOpt.Element, optTarget.Element);
                    if (IsOptional)
                        return Algebra.StateExtensions.FitsInto(Descendant, optTarget.Element);
                    return false;
                }
                return Algebra.StateExtensions.FitsInto(Descendant, type);
            }
            default:
                return false;
        }
    ...
    // Delegate to the depth-guarded printer: naive interpolation of Descendant/Ancestor
    // recurses forever on μ-recursive states (CS ↔ StateFun ToString cycle).
