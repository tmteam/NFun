...
namespace NFun.Interpritation
{
    class SingleExpressionReader
    {
        ...
        {
            if(node.Is(SyntaxNodeType.Var))
                return GetOrAddVariableNode(node);
            if(node.Is(SyntaxNodeType.Fun))
                return GetFunNode(node);
            if(node.Is(SyntaxNodeType.IfThanElse))
                return GetIfThanElseNode(node);
            if(node.Is(SyntaxNodeType.Number))
                return GetValueNode(node);
            if(node.Is(SyntaxNodeType.Text))
                return GetTextValueNode(node);
            if(node.Is(SyntaxNodeType.ArrayInit))
                return GetArrayNode(node);
            if(node.Is(SyntaxNodeType.ProcArrayInit))
                return GetProcedureArrayNode(node);
            if(node.Is(SyntaxNodeType.AnonymFun))
