...
namespace NFun.SyntaxTests
{
    ...
    public class AnonymousFunTest
    {
        ...
        [TestCase("y = [-7,-2,0,1,2,3].filter(fun it>0)", new[] { 1.0, 2.0, 3.0 })]
        [TestCase("y = [-1,-2,0,1,2,3].filter((fun it>0)).fold(fun(i,j)= i+j)", 6.0 )]
        [TestCase("y = [-1,-2,0,1,2,3].filter((fun it>0)).filter(fun(i)=i>2)", new[]{3.0})]
        ...
        [TestCase("y = [-1,-2,0,1,2,3].filter((fun it>0)).fold(fun(a,b)= a+b)", 6.0 )]
        [TestCase("y = [-1,-2,0,1,2,3].filter((fun it>0)).filter(fun(a)=a>2)", new[]{3.0})]
        [TestCase("y = [-1,-2,0,1,2,3].filter((fun it>0)).map(fun(a)=a*a).map(fun(b)=b*b)", new[]{1.0,16.0,81.0})]
        ...
        [TestCase(@"car3(g) = g(2);   y = car3(fun(x)=x-1)   ", 1.0)]
        [TestCase(@"car4(g) = g(2);   y = car4(fun it)   ", 2.0)]
        [TestCase(@"call5(f, x) = f(x); y = call5(fun(x)=x+1,  1)", 2.0)]
        [TestCase(@"call6(f, x) = f(x); y = call6(fun(x)=x+1.0, 1.0)", 2.0)]
        [TestCase(@"call7(f, x) = f(x); y = call7(fun(x:real)=x+1.0, 1.0)", 2.0)]
        [TestCase(@"call8(f) = fun(i)=f(i); y = call8(fun(x)=x+1)(2)", 3.0)]
        [TestCase(@"call9(f) = fun(i)=f(i); y = (fun(x)=x+1).call9()(2)", 3.0)]
        [TestCase(@"mult(x)= fun(y)=fun(z)=x*y*z;    y = mult(2)(3)(4)", 24.0)]
        ...
        [TestCase(@"y = [1,2,3].fold(fun it1+it2)", 6.0)]
        ...
        [TestCase("y = [-7,-2,0,1,2,3].filter (fun it>0)", new[] { 1.0, 2.0, 3.0 })]
        [TestCase("y = [-1,-2,0,1,2,3].filter (fun it>0) .fold(fun it1+it2)", 6.0 )]
        [TestCase("y = [-1,-2,0,1,2,3].filter (fun it>0).filter(fun it>2)", new[]{3.0})]
        [TestCase("y:int[] = [-1,-2,0,1,2,3].filter (fun it>0).map(fun it*it).map(fun it*it)", new[]{1,16,81})]
        [TestCase("y = [-1,-2,0,1,2,3].filter (fun it>0).map(fun it*it).map(fun it*it)", new[]{1.0,16.0,81.0})]
        ...
        [TestCase(@"call5(f, x) = f(x); y = call5((fun it+1),  1)", 2.0)]
        [TestCase(@"call6(f, x) = f(x); y = call6((fun it+1.0), 1.0)", 2.0)]

        [TestCase(@"call8(f) = (fun f(it)); y = call8((fun it+1))(2)", 3.0)]
        [TestCase(@"call9(f) = (fun f(it)); y = ((fun it+1)).call9()(2)", 3.0)]

        [TestCase(@"call10(f,x) = (fun f(x,it)); y =  max.call10(3)(2)", 3.0)]
        [TestCase(@"call11() = fun it; y =  call11()(2)", 2.0)]
        [TestCase(@"call12 = (fun it); y =  call12(2)", 2.0)]
        ...
        [TestCase(@"y = [[1,2],[3,4],[5,6]].map(fun it.map(fun it+x).sum()).sum()", 1.0, 27.0)]
