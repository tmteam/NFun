    ...
    public class StructConcreteFunctionsTest
    {
        ...
        public void CallConcreteNestedFunctionMultipleFieldOfTextAccess() =>
            ("f(x):text = x.a.m1.concat(x.b.m2);" +
             "x1 = the{a = the{m1='mama'}; b = the{m2='popo'}};" +
             "r = f(x1);").AssertResultHas("r", "mamapopo");
        ...
        ("f(x:uint32) = the{twice = x+x; dec = x-1}; " +
         "t = f(42).twice;" +
        ...
        public void ConcreteFactorialReq_ReturnStruct(int x, int y) =>
            @"fact(n:int) = if(n<=1) the{res = 1} else the{res = fact(n-1).res*n }
        ...
        public void ConcreteFactorialReq_ArgIsStruct(int x, int y) =>
            @"fact(n):int = if(n.field<=1) 1 else fact(the{field=n.field-1}) * n.field;
        ...
        [TestCase("f(x):int = x.a; y = f(the{x = true})")]
        [TestCase("f(x):int = x.a; y:bool = f(the{missing = 1})")]
        [TestCase("f(x):real = the{res = x}")]
        [TestCase(@"fact(n):int = if(n.field<=1) 1 else fact(the{field=n.field-1}) * n.field;
                  y = fact(the{a=x})")]
