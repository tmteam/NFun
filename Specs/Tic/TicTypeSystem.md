...
Абстрактные типы **сохраняют информацию** при вычислении LCA/GCD для пар signed/unsigned. Если constraint-интервал коллапсирует к абстрактному типу — это ошибка (типы несовместимы). Пофазная легальность коллапса абстрактных границ (что отвергает `SimplifyOrNull`, что и как материализуется на Finalize) нормативно описана в `TicAlgorithm_Destruction.md` §6d.
