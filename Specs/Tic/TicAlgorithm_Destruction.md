...
**Stale snapshot и DescendantHasOptionalLift — WORKAROUND.** Desc-snapshot в CS создаётся во время Pull (Phase 1), но фактическое состояние descendant может измениться после Phase 2 (PullNoneNode оборачивает элементы в Optional). Snapshot в CS устаревает. Два специальных случая в Destruction **обходят** это расхождение:
1. **Actual descendant содержит Optional-wrapped элементы, которых нет в snapshot** (e.g., snapshot `arr(U8)`, actual `arr(opt(U8))`): ancestor принимает actual descendant через RefTo (snapshot устарел, actual точнее).
2. **Snapshot содержит Optional-элементы, которых нет в actual** (e.g., snapshot `arr(opt(U8))` от IsOptional LCA, actual `arr(U8)`): ancestor принимает snapshot (включающий Optional), затем рекурсивная Destruction по элементам.

**Корневая причина**: двухфазный Pull создаёт temporal gap — между Phase 1 (snapshot) и Phase 2 (Optional wrapping). Чистое решение: single-pass Pull с immediate Optional propagation, или invalidation/refresh снапшотов после Phase 2. Текущий workaround корректен (покрыт тестами) но добавляет complexity в Destruction.
...
**Redirect SyntaxNode-ов.** После `CS(opt) → Opt(innerNode)`, MaterializeOptionalNode сканирует все узлы графа: если SyntaxNode имеет `RefTo` на материализованный узел, ссылка перенаправляется на innerNode. Цель: конкретные значения (константы, литералы) должны сохранить свой non-Optional тип (e.g., `42` остаётся I32, а не Opt(I32)). Без redirect SyntaxNode разрешился бы к Optional-типу, что приведёт к некорректному boxing в runtime.
...
1. Preferred в интервале → Preferred (пропускается при ignorePreferred=true для function signatures)
2. IsComparable + Anc не comparable → unresolved (остаётся CS)
3. Desc — composite (Array, Struct, Fun, Optional) → Desc (composite информативнее чем Anc)
4. Desc — abstract primitive (I48, U24...) без Anc → ConcreteAncestor (I48→I64, U24→U32, ...)
5. Anc определён → Anc (самый широкий допустимый)
6. Пусто → Any

Если IsOptional: результат оборачивается в `Opt(inner)` (кроме Any → Any без обёртки).

**Контравариантная** (`SolveContravariant`, аргументы функций): приоритет:
1. Preferred в интервале → Preferred (всегда, даже при ignorePreferred — contravariant всегда использует Preferred)
2. Нет Desc → unresolved (остаётся CS)
3. IsComparable + Desc не comparable → unresolved
4. Иначе → Desc (самый узкий)

Обоснование: аргументы функций разрешаются к конкретному типу для мономорфизации. Preferred обеспечивает предсказуемость (I32 вместо U8 для integer args).
...
**SolveContravariant**: T = Preferred (если в интервале) или Desc (самый узкий). `D ≤ T ≤ A` — Preferred: аналогично случаю 1. Desc: `D ≤ D` — тривиально. ∎
