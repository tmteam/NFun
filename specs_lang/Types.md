...
### Collections and `text` (strings)

A collection is an ordered or hashed group of elements of the same type. `int[]` is a mutable, element-invariant `array<int>` — `a[i] = v` works.
...
You can read more about collections in the boring **Collections** section.
...
| Identifier      | Mutability            | Description                                                              |
|-----------------|-----------------------|--------------------------------------------------------------------------|
...
A type class is a named subset of types used as a generic constraint.

| Name        | Members                                                            | Used by                                              |
|-------------|--------------------------------------------------------------------|------------------------------------------------------|
| Comparables | `text`, `char`, all integer types, `real`                          | `<`, `<=`, `>`, `>=`, `min`, `max`, `sort`, `median` |
| Immutable   | `bool`, all integer types, `real`, `char`, `text`, `ip`            | `set<T>` element, `map<K,V>` key                     |
| Clearable   | `list<T>`, `set<T>`, `map<K,V>`                                    | `clear`                                              |
| Enumerable  | `T[]`, `list<T>`, `array<T>`, `fixedArray<T>`, `set<T>`, `map<K,V>` | LINQ — `map`, `filter`, `fold`, `count`, …           |

A call violating a type class is rejected at compile time: `sort([{x=1}])` fails (struct is not Comparable), `set({x=1})` fails with FU580 (struct is not Immutable).
